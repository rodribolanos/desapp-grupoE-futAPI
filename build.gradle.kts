...
    violationRules {
        rule {
            // Queremos que la regla se aplique a todo el proyecto (BUNDLE)
            element = "BUNDLE"
            
            // Definimos un límite para las LÍS (LINE)
            limit {
                counter = "LINE"
                value = "COVEREDRATIO" // Usamos el ratio (0.0 a 1.0)
...
    // para no medir DTOs, Configs, etc.
    classDirectories.setFrom(files("$buildDir/classes/kotlin/main"))
    sourceDirectories.setFrom(files("src/main/kotlin"))
    executionData.setFrom(fileTree(buildDir) {
        include("**/jacoco/test.exec")
...
// Este bloque aplica los filtros a la tarea de validación
afterEvaluate {
    tasks.named<JacocoCoverageVerification>("jacocoTestCoverageVerification") {
        classDirectories.setFrom(files(classDirectories.files.map {
            fileTree(it) {
